...
/** Popular Voca: Web Speech API(브라우저 TTS) — 백엔드 미사용 */
...
function pvStripTagsToText(html) {
    const d = document.createElement('div');
    d.innerHTML = html;
    return d.textContent.replace(/\s+/g, ' ').trim();
}

function pvIsMostlyEnglish(text) {
...
function attachPopularVocaWebTTS(container) {
    ...
    paragraphs.forEach((p) => {
        if (p.getAttribute('data-pv-tts') === '1') return;
        if (!/예문/.test(p.textContent)) return;

        const raw = p.innerHTML;
        const lines = raw.split(/<br\s*\/?>/i);
        const newsIdx = lines.findIndex((l) => /뉴스\s*예문/i.test(pvStripTagsToText(l)));

        const newLines = lines.map((lineHtml, i) => {
...
function repairBrokenLinks(html) {
    ...
    // 1) JSON 이스케이프된 따옴표로 깨진 경우: href=\"URL\">기사 보기 → <a href="URL">기사 보기</a>
    out = out.replace(/href=\\"(https:\/\/www\.wsj\.com\/[^"\\]+)\\"\s*>\s*기사 보기/g, '<a href="$1">기사 보기</a>');
    out = out.replace(/href=\\"(https:\/\/[^"\\]*(?:youtube\.com|youtu\.be)[^"\\]*)\\"\s*>\s*예문 영상 보기/g, '<a href="$1">예문 영상 보기</a>');

    // 2) URL이 그대로 있고 "> 만 잘린 경우: URL">기사 보기 → <a href="URL">기사 보기</a>
    out = out.replace(/(https:\/\/www\.wsj\.com\/[^"<>\s]+)\s*"\s*>\s*기사 보기/g, '<a href="$1">기사 보기</a>');
    out = out.replace(/(https:\/\/[^"<>\s]*(?:youtube\.com|youtu\.be)[^"<>\s]*)\s*"\s*>\s*예문 영상 보기/g, '<a href="$1">예문 영상 보기</a>');
...
// 에러 메시지 표시 함수
function showError(message, details = '', listPage = 'situational-english-list.html') {
    const postContainer = document.getElementById('post-container');
...
async function loadPost() {
    ...
    try {
        const url = `${API_BASE_URL}/situational-english`;
        ...
        // 조회수 증가 API 호출
        try {
            const viewResponse = await fetch(`${API_BASE_URL}/situational-english/${post._id}/view`, {
                method: 'POST',
                headers: {
